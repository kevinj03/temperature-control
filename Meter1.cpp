...
CMeter::CMeter()
{
	...
	// 颜色表格
	m_colorTable[0] = RGB(0, 255, 255);
	m_colorTable[1] = RGB(  160, 255,   160);
	m_colorTable[2] = RGB(255,   233,   90);
	m_colorTable[3] = RGB(255,180, 34);
...
void CMeter::OnPaint() 
{
	...
	// 选取圆盘边框半径
	m_nRadiusFrame = max(m_rectCtrl.Height(), m_rectCtrl.Width())/3;
...
{
	int nInnerRadius = m_nRadiusFrame*8/10;	// 内圆弧半径  m_nRadiusFrame仪表盘边框半径
	...
	ptStart.y = m_ptMeterCenter.y+(int)(m_nRadiusFrame*sin(m_dLeftAngleRad));// m_dLeftAngleRad起始角弧度
	ptEnd.x = m_ptMeterCenter.x+(int)(m_nRadiusFrame*cos(-m_dRightAngleRad));//仪表盘中心点
	...
	for(int iOnBand=nFrame; iOnBand>0; iOnBand--)//nFrame边框厚度
	{
		...
		pDC->Arc(&rectPanel, ptEnd, ptStart);//arc
		pDC->SelectObject(pPenOld);	//SelectObject替换
	}
	// 绘制内圈
	ptInnerStart.x = m_ptMeterCenter.x-(int)(nInnerRadius*cos(m_dLeftAngleRad));//内圈起始点坐标
	ptInnerStart.y = m_ptMeterCenter.y+(int)(nInnerRadius*sin(m_dLeftAngleRad));
	ptInnerEnd.x = m_ptMeterCenter.x+(int)(nInnerRadius*cos(-m_dRightAngleRad));//内圈终止点坐标
	ptInnerEnd.y = m_ptMeterCenter.y+(int)(nInnerRadius*sin(-m_dRightAngleRad));
	
	rectInnerPanel.SetRect(m_ptMeterCenter.x-nInnerRadius, m_ptMeterCenter.y-nInnerRadius,		//  m_ptMeterCenter仪表中心
						m_ptMeterCenter.x+nInnerRadius ,m_ptMeterCenter.y+nInnerRadius);

	penDraw.DeleteObject();
	penDraw.CreatePen(PS_SOLID, 1, RGB(255,255,0));
	pPenOld = pDC->SelectObject(&penDraw);
	pDC->Arc(&rectInnerPanel, ptInnerEnd, ptInnerStart);//arc
...
*/
	// 绘制刻度
	penDraw.DeleteObject();
	penDraw.CreatePen(PS_SOLID, 1, RGB(255,0,0));
	pPenOld = pDC->SelectObject(&penDraw);
	for(i=0; i<nRef; i++)
	{
